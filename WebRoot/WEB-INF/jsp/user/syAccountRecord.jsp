				...
				<div class="jui_list_cells">
					<c:forEach items="${accountRecordExpands }" var="record">
						<div class="jui_list_cell">
							<div class="info">
								<p>
									变更时间：<span><fmt:formatDate
											value="${record.operationTime }" pattern="yyyy-MM-dd" /></span>
								</p>
								<p>
									聚财宝数量：<span>${record.dlbNum }</span>
								</p>
								<p>
									存入金额：<span>${record.distributedMoney }</span>
								</p>
								<p>
									存入养老金：<span>${record.distributedYlj }</span>
								</p>
							</div>
							<span class="changeVal modelUp">
								+${record.distributedMoney + record.distributedYlj}
							</span>
						</div>
					</c:forEach>
				</div>
