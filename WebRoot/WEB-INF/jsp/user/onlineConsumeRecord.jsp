							...
							<div class="info">
								<p>
									消费时间：<span><fmt:formatDate
											value="${record.onlineJournalBook.journalTime }" pattern="yyyy-MM-dd HH:mm:ss" /></span>
								</p>
								<p>
									商品：<span>${record.onlineCommodityModel.onlineCommodity.commodityName }</span>
								</p>
								<p>
									型号：<span>${record.onlineCommodityModel.commodityModel }</span>
								</p>
								<p>
									成交价：<span>${record.amount }</span>
								</p>
								<p>
									购买数量：<span>${record.num }</span>
								</p>
							</div>
						</div>
					</c:forEach>
